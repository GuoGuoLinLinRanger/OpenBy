...
type SearchTrendSectionProps = {
  ...
  error?: string;
  /** Keyword used for the API query (e.g. category like "laptop") - used for the link */
  keywordUsed?: string;
  /** Product name for citation - shown to users so they know which product this refers to */
  productName?: string;
  /** Top 8 related search queries - with search buttons linking to Google */
    ...
    <section id={id} className="relative overflow-hidden border-b border-zinc-200/80 py-16">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        {relatedSearches.length > 0 && (
          <div className="mb-8 rounded-xl border border-zinc-200/80 bg-white/80 p-5 shadow-sm">
            <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-zinc-700">
              <Search className="h-4 w-4 text-indigo-600" />
              Top things people search about this product
            </h3>
            <div className="flex flex-wrap gap-2">
              {relatedSearches.slice(0, 8).map((query, i) => (
                <a
                  key={i}
                  href={`https://www.google.com/search?q=${encodeURIComponent(query)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-lg border border-zinc-200/80 bg-zinc-100/80 px-3 py-2 text-sm font-medium text-zinc-800 transition-colors hover:bg-zinc-200/80 hover:border-zinc-300/80"
                >
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-stretch lg:gap-10">
          ...
          <div className="flex shrink-0 flex-col items-center justify-center gap-4 rounded-2xl border border-zinc-200/80 bg-white/80 px-8 py-8 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_12px_rgba(0,0,0,0.03)] backdrop-blur-sm lg:w-56">
            ...
            >
              {(score / 10).toFixed(1)}
              <span className="text-[0.5em] font-normal opacity-90 [vertical-align:-0.4em]">/10</span>
            </span>
            {error && (
              ...
              <ResponsiveContainer width="100%" height={280}>
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [Number(value ?? 0), "Interest"]}
