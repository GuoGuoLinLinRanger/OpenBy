...
function TypingMessage({ content }: { content: string }) {
  ...
  const [done, setDone] = useState(content.length === 0);
...
export function ProductQASection({ productTitle, productDescription }: ProductQASectionProps) {
  ...
  return (
    <section className="relative z-20 overflow-visible border-b border-zinc-200/80 py-16">
      {/* Background - gradients pushed to edges to avoid blending with center text */}
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-50/40 via-white to-violet-50/30" />
      <div
      ...
      />
      <div className="absolute right-0 top-0 h-2/3 w-2/3 bg-[radial-gradient(ellipse_80%_80%_at_100%_0%,rgba(99,102,241,0.07),transparent_60%)]" />
      <div className="absolute bottom-0 left-0 h-2/3 w-2/3 bg-[radial-gradient(ellipse_80%_80%_at_0%_100%,rgba(139,92,246,0.05),transparent_60%)]" />
