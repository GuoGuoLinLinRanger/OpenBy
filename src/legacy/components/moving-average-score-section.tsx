...
export function MovingAverageScoreSection({
  ...
  const allPrices = chartData.flatMap((d) => [
    d.price,
    ...(d["MA(7)"] != null ? [d["MA(7)"]] : []),
    ...(d["MA(60)"] != null ? [d["MA(60)"]] : []),
  ]).filter((p): p is number => typeof p === "number" && p > 0);
  const dataMin = allPrices.length > 0 ? Math.min(...allPrices) : 0;
  const dataMax = allPrices.length > 0 ? Math.max(...allPrices) : 100;
  const range = Math.max(dataMax - dataMin, dataMin * 0.02);
  const padding = range * 0.08;
  const yDomain = [Math.max(0, dataMin - padding), dataMax + padding] as [number, number];
